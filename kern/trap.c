...
void page_fault_handler(struct Env * curenv, uint32 fault_va) {
	...
	int cur_size_ws = LIST_SIZE(&(curenv->PageWorkingSetList));
	int max_size_ws = curenv->page_WS_max_size;

	//Working setList not Full
	if (cur_size_ws != 0) {

		//PLACEMENT
		//check if in SC List
		struct WorkingSetElement *search_element;
		LIST_FOREACH(search_element, &(curenv->SecondList))
		{
			if (search_element->virtual_address == fault_va) {
				LIST_REMOVE(&(curenv->SecondList), search_element);

				struct WorkingSetElement * last_element_AC;
				last_element_AC = LIST_LAST(&(curenv->ActiveList));

				LIST_REMOVE(&(curenv->ActiveList), last_element_AC);
				last_element_AC->empty = 1;

				LIST_INSERT_HEAD(&(curenv->SecondList), last_element_AC);
				pt_set_page_permissions(curenv,
						last_element_AC->virtual_address, 0, PERM_PRESENT);

				LIST_INSERT_HEAD(&(curenv->ActiveList), search_element);
		...
		//check if active list FULL then free place
		int cur_size_aclist = LIST_SIZE(&(curenv->ActiveList));

		if (cur_size_aclist >= curenv->ActiveListSize) {
			...
			LIST_INSERT_HEAD(&(curenv->SecondList), last_element_AC);
			pt_set_page_permissions(curenv, last_element_AC->virtual_address, 0,
			PERM_PRESENT);
		}

		//Active NOT full
		struct WorkingSetElement * new_element;
		new_element = LIST_LAST(&(curenv->PageWorkingSetList));
		...
		if (ret != E_NO_MEM) {
			...
			ret = map_frame(curenv->env_page_directory, frame_info,
					(void*) fault_va, PERM_USER | PERM_WRITEABLE);
		...
		int rret = pf_read_env_page(curenv, (void *) fault_va);
		if (rret == E_PAGE_NOT_EXIST_IN_PF) {
		...
		//check if second list empty or not exist
		int cur_size_sclist = LIST_SIZE(&(curenv->SecondList));
		if (curenv->SecondListSize == 0 || cur_size_sclist == 0) {

			struct WorkingSetElement * victim_element;
			victim_element = LIST_LAST(&(curenv->ActiveList));

			uint32 last_element_AC_perm = pt_get_page_permissions(curenv,
					victim_element->virtual_address);

			if (last_element_AC_perm & PERM_MODIFIED)

			{
				uint32 * env_page_table;
				struct Frame_Info * modified_page_frame_info;

				modified_page_frame_info = get_frame_info(
						curenv->env_page_directory,
						(void *) victim_element->virtual_address,
						&env_page_table);
			...
			LIST_REMOVE(&(curenv->ActiveList), victim_element);
			victim_element->empty = 1;

			LIST_INSERT_HEAD(&(curenv->PageWorkingSetList), victim_element);
			victim_element->empty = 1;
			...
			//add in active list
			struct WorkingSetElement * new_element;
			new_element = LIST_FIRST(&(curenv->PageWorkingSetList));
		...
		//active list and second list are full
		struct WorkingSetElement * victim_element;

		victim_element = LIST_LAST(&(curenv->SecondList));

		LIST_REMOVE(&(curenv->SecondList), victim_element);

		uint32 last_element_SC_perm = pt_get_page_permissions(curenv,
				victim_element->virtual_address);

		if (last_element_SC_perm & PERM_MODIFIED)

		{
			...
			modified_page_frame_info = get_frame_info(
					curenv->env_page_directory,
					(void *) victim_element->virtual_address, &env_page_table);
