...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	int N = ROUNDUP(size, (PAGE_SIZE));
	int number_of_pages = N / (PAGE_SIZE);
	uint32 adress = virtual_address;	 // virtual address

	for (int i = 0; i < number_of_pages; i++) {
		int r = pf_add_empty_env_page(e, adress, 0);
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	uint32 v = virtual_address;
	int it = ROUNDUP(size, PAGE_SIZE) / PAGE_SIZE;
	...
	for (int i = 0; i < it; i++) {
		...
		{
			if (v == element->virtual_address) {
				LIST_REMOVE(&(e->ActiveList), element);

				//Insert the removed element from active in the PageWorkingSetList
				LIST_INSERT_HEAD(&(e->PageWorkingSetList), element);
				element->empty = 1;
				struct WorkingSetElement *element2;
				if ( LIST_SIZE(&(e->SecondList)) >= 1) {
					element2 = LIST_FIRST(&(e->SecondList));
				...
				//set to present
				pt_set_page_permissions(e, v, PERM_PRESENT, 0);

				unmap_frame(e->env_page_directory, (void*) v);
	...
	for (int i = 0; i < it; i++) {
		ptr = NULL;
		int found = 1;
		get_page_table(e->env_page_directory, (void*) v, &ptr);
		if (ptr != NULL) {
			...
			if (found) {
				...
				struct Frame_Info *frame_info = to_frame_info(
						(e->env_page_directory[PDX(v)] >> 12) << 12);

				frame_info->references--;

				if (frame_info->references == 0)
					free_frame(frame_info);
				pd_clear_page_dir_entry(e, (uint32) v);
